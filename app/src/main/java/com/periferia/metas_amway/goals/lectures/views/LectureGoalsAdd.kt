...
class LectureGoalsAdd : Fragment(), DatePickerDialog.OnDateSetListener {
    ...
    private fun saveData() {
        ...
        AsyncTask.execute {
            ...
            db.bookDao().setCurrentPage(book.currentPage, book.id!!)
        }

        val list:Fragment = LectureGolasList()
        val transaction = fragmentManager!!.beginTransaction()
        list.onDetach()
        transaction.replace(R.id.contenedor_principal, list).addToBackStack(null).commit()
    }
